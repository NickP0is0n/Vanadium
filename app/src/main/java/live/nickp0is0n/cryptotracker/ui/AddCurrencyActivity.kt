...
class AddCurrencyActivity : AppCompatActivity() {
    ...
    fun onSearchButtonClick(view: View) {
        ...
        lifecycleScope.launch {
            val reply = coinList.firstOrNull { it.equals(request, ignoreCase = true) }
            if (reply == null) {
                ...
                activeCurrency = getCryptoCurrencyFromCoinData(service.getCoinById(reply.toLowerCase(
                    Locale.ROOT
                ).replace(" ", "-"), marketData = true))
                runOnUiThread {
